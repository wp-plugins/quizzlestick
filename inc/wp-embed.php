...
class WP_Custom_Embed {
	 ...
	 * Constructor
	 */
	function __construct( $filter = 'the_content' ) {
		global $wp_embed;

		// copy original class settings
		$this->handlers = $wp_embed->handlers;
		$this->post_ID = $wp_embed->post_ID;
		$this->usecache = $wp_embed->usecache;
		$this->linkifunknown = $wp_embed->linkifunknown;

		// Hack to get the [embed] shortcode to run before wpautop()
		add_filter( $filter, array( $this, 'run_shortcode' ), 8 );

		// Shortcode placeholder for strip_shortcodes()
		add_shortcode( 'embed', '__return_false' );

		// Attempts to embed all URLs in a post
		add_filter( $filter, array( $this, 'autoembed' ), 8 );
